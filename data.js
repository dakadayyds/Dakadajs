...
class dakada{    
        ...
        return {
            ...
            blocks: [
                ...
                },
                {
                    opcode: 'js_coder',
                    blockType: Scratch.BlockType.COMMAND,
                    text: '将js代码[list]转换成javascript函数，函数名：[name]',
                    arguments: {
                        list: {
                            type: Scratch.ArgumentType.STRING,
                            defaultValue: '?',
    ...
    coder_command(args2){
        (new Function(args2.code2))();
    }
    js_coder(arg){
        if (this.listnames.indexOf(arg.name)>-1){
            try{
            ...
            try{
                this.listnames.push(arg.name);
                this.lists.push(new Function("arg",arg.list));
            }catch(Err2){
                this.e=Err2
            }
        }
    ...
    run_fuction(arg){
        try{
            let thisparam=new String(arg.param)
            this.e=''
            return this.lists[this.listnames.indexOf(arg.name)](thisparam.split(','))
        }catch(error){
            this.e=error;
            return "unexpected"
        }
